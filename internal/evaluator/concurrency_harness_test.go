...
func BenchmarkHTTPServeThroughput(b *testing.B) {
	...
	srv := startServerTB(b, 64, e.httpHandler(handler, nil, nil))
	defer srv.Close()
	client := srv.Client()
...
func TestHTTPConcurrencyLatency(t *testing.T) {
	...
	srv := startServerTB(t, concurrency, e.httpHandler(handler, nil, nil))
	defer srv.Close()
	client := srv.Client()
...
func TestHTTPServerWorkerPoolOverload(t *testing.T) {
	...
	srv := startServerTB(t, 16, e.httpHandler(handler, pool, nil))
	defer srv.Close()
	client := srv.Client()
...
func TestServerHandlerStateIsolation(t *testing.T) {
	...
	srv := startServerTB(t, 1, e.httpHandler(handler, nil, nil))
	defer srv.Close()
