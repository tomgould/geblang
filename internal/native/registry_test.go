...
func TestParseErrorValue(t *testing.T) {
	...
	if dict.Entries["smessage"].Value.Inspect() != "bad data" {
		t.Fatalf("message: got %q", dict.Entries["smessage"].Value.Inspect())
	}
	if dict.Entries["sline"].Value.Inspect() != "2" {
		t.Fatalf("line: got %q", dict.Entries["sline"].Value.Inspect())
	}
}
